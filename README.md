...
> [!NOTE]
> По умолчанию приложение работает локально в **демо-режиме без кошелька**. Web3-контур
> подключается к Stellar testnet и не требует реальных средств.
...
- **Автоматическая проверка** — ESLint, TypeScript, 38 тестов и Webpack-сборка запускаются в CI.
...
## Переключение демо- и Web3-режима

По умолчанию приложение запускается в демо-режиме: кнопка **«Начать играть»** сразу открывает
игру без Albedo, а покупки и полив используют виртуальный баланс и локальное хранилище браузера.
Отдельной кнопки демо-режима в интерфейсе нет. При каждом новом запуске демо старые покупки,
история полива и виртуальный баланс удаляются: показ всегда начинается с чистого сада и 1000 XLM.

Режим выбирается в коде одной константой:
[`src/constants/gameMode.ts`](src/constants/gameMode.ts).

```ts
// 'demo' — без кошелька; 'wallet' — Albedo и Stellar testnet
export const GAME_ACCESS_MODE: GameAccessMode = 'demo';
```

Чтобы включить Web3-контур, замените `'demo'` на `'wallet'`, затем перезапустите dev-сервер или
пересоберите приложение. При переходе с демо на Web3-режим сохранённая демо-сессия автоматически
игнорируется; реальный кошелёк подключается через обычное модальное окно Albedo.
...
Текущее покрытие сценариев: **12 test suites · 38 тестов**.
